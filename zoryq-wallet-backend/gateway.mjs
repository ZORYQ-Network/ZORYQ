...
const server=http.createServer(async(req,res)=>{
 ...
 if(u.pathname==='/gateway/health')return void (res.writeHead(200,{'content-type':'application/json','cache-control':'no-store'}),res.end(JSON.stringify({ok:true,service:'zoryq-wallet-gateway',swapWorker:child.exitCode===null,socialPrefix:'/social',communityPrefix:'/community',messagePrefix:'/message',mediaUpload:true})));
 if(u.pathname.startsWith('/message')){await handleMessages(req,res,u);return}
...
server.listen(PORT,'0.0.0.0',()=>console.log(`[zoryq-gateway] listening on :${PORT}; swap worker :${SWAP_PORT}; social=/social; communities=/community; messages=/message`));
