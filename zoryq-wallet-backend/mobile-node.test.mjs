...
import { MobileNodeProtocol, nodeIdFromPublicKey, registrationPayload, proofPayload, ZORYQ_CHAIN_ID } from './mobile-node.mjs';
...
function register(f, id) {
  ...
  const full = { ...c, publicKey: id.publicKey };
  return f.protocol.register({ challengeId: c.id, nodeId: id.nodeId, publicKey: id.publicKey, signature: id.sign(registrationPayload(full)) });
}
...
test('registration requires a valid dedicated node signature', () => {
  ...
  const full = { ...c, publicKey: id.publicKey };
  assert.equal(f.protocol.register({ challengeId: c.id, nodeId: id.nodeId, publicKey: id.publicKey, signature: id.sign(registrationPayload(full)) }).ok, true);
});
...
test('verified block proof awards XP exactly once and replay is rejected', async () => {
  ...
  const proof = {
    challengeId: c.id, nonce: c.nonce, nodeId: id.nodeId, chainId: ZORYQ_CHAIN_ID, blockNumber: c.targetBlock,
    blockHash: '0x' + 'ab'.repeat(32), parentHash: '0x' + 'cd'.repeat(32), observedAt: new Date(f.now).toISOString()
  };
  proof.signature = id.sign(proofPayload(proof));
  const accepted = await f.protocol.submitProof(proof);
  assert.equal(accepted.xpAwarded, 3);
  assert.equal(accepted.totalXp, 3);
  await assert.rejects(() => f.protocol.submitProof(proof), /CHALLENGE_ALREADY_USED/);
  assert.equal(f.protocol.status(id.nodeId).totalXp, 3);
...
test('wrong block data earns zero XP', async () => {
  ...
  const proof = {
    challengeId: c.id, nonce: c.nonce, nodeId: id.nodeId, chainId: ZORYQ_CHAIN_ID, blockNumber: c.targetBlock,
    blockHash: '0x' + 'ee'.repeat(32), parentHash: '0x' + 'cd'.repeat(32), observedAt: new Date(f.now).toISOString()
  };
  proof.signature = id.sign(proofPayload(proof));
...
test('expired challenge earns zero XP', async () => {
  ...
  const proof = {
    challengeId: c.id, nonce: c.nonce, nodeId: id.nodeId, chainId: ZORYQ_CHAIN_ID, blockNumber: c.targetBlock,
    blockHash: '0x' + 'ab'.repeat(32), parentHash: '0x' + 'cd'.repeat(32), observedAt: c.issuedAt
  };
  proof.signature = id.sign(proofPayload(proof));
...
test('append-only ledger reconstructs XP and consumed challenge after restart', async () => {
  const f = fixture();
  const id = identity();
  register(f, id);
  const c = await f.protocol.taskChallenge(id.nodeId);
  ...
  assert.equal(restarted.status(id.nodeId).totalXp, 3);
