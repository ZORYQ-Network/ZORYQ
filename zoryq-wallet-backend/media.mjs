...
const PUBLIC_BASE=String(process.env.SOCIAL_PUBLIC_BASE_URL||'').replace(/\/$/,'');
...
function canonicalBase(req){if(PUBLIC_BASE)return PUBLIC_BASE;const proto=String(req.headers['x-forwarded-proto']||'https').split(',')[0];const host=String(req.headers['x-forwarded-host']||req.headers.host||'').split(',')[0];if(!host)throw Error('PUBLIC_HOST_UNAVAILABLE');return `${proto}://${host}`}
...
export async function handleSocialMedia(req,res,u){
 ...
 if(req.method==='POST'&&p==='/v1/media/image'){
  ...
  try{
   ...
   fs.mkdirSync(DATA_DIR,{recursive:true});const name=`${crypto.randomBytes(16).toString('hex')}.${type.ext}`;const file=path.join(DATA_DIR,name);fs.writeFileSync(file,data,{flag:'wx',mode:0o640});const mediaPath=`/social/media/${name}`;const url=`${canonicalBase(req)}${mediaPath}`;json(res,201,{ok:true,actor,url,path:mediaPath,mime,size:data.length});return true
