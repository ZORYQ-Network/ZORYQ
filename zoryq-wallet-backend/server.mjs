...
const server=http.createServer(async(req,res)=>{
 ...
 if(req.method==='GET'&&u.pathname==='/health')return send(res,ready()?200:503,{ok:ready(),service:'zoryq-wallet-swap-backend',provider:'0x-swap-api-v2',feeBps:FEE_BPS,treasuryConfigured:ADDRESS.test(TREASURY),apiConfigured:ZEROX_API_KEY.length>10,supportedChainIds:[...SUPPORTED]});
 if(req.method==='POST'&&u.pathname==='/quote'){
  ...
  }catch(e){return send(res,400,{ok:false,error:e?.message||'INVALID_REQUEST'})}
 }
