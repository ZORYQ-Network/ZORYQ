...
function Root(){
  ...
  async function saveSecurity(next){
    securityConfig=next;setSecurity(next);await AsyncStorage.setItem(SECURITY_KEY,JSON.stringify(next));
  }
  async function toggleSecurity(key){
    ...
    if(!security[key]){
      const ok=await authenticate(key==='txConfirm'?'Ativar confirmação de transações':'Ativar desbloqueio do app');
      if(!ok)return;
    }
