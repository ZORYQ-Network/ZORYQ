...
export type SocialProfile={id:string;username:string|null;display_name:string|null;bio:string|null;avatar_url:string|null;reputation_score:number|null;interests:string[]|null;power_key:string|null;human_score:number|null;profile_visibility:string|null;discoverable:boolean|null;show_reputation:boolean|null};
...
export async function getBackendState():Promise<BackendState>{const {data}=await socialSupabase.auth.getSession();return {configured:Boolean(SUPABASE_URL&&SUPABASE_PUBLISHABLE_KEY),authenticated:Boolean(data.session),session:data.session}}
...
export function subscribeSocial(onChange:()=>void,userId?:string):RealtimeChannel{
 ...
 if(userId)channel.on('postgres_changes',{event:'*',schema:'public',table:'notifications',filter:`user_id=eq.${userId}`},onChange).on('postgres_changes',{event:'*',schema:'public',table:'user_follows'},onChange).on('postgres_changes',{event:'*',schema:'public',table:'messages'},onChange).on('postgres_changes',{event:'*',schema:'public',table:'xp_events',filter:`user_id=eq.${userId}`},onChange);
