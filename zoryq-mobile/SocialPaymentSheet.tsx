 ...
 return <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
  <View style={s.back}><View style={s.sheet}>
   ...
   <View style={s.summary}>
    <View style={s.row}><Text style={s.key}>Destinatário</Text><Text style={s.value}>{short(target?.address||'')}</Text></View>
   ...
   <Text style={s.note}>A ZORIQ não fica com seus fundos. O contrato divide a operação atomicamente: perfil + Treasury. Sua seed e chave privada nunca são enviadas ao servidor.</Text>
  </View></View>
 </Modal>
}
function short(v:string){return v&&v.length>18?`${v.slice(0,8)}…${v.slice(-6)}`:v}
function friendly(m:string){if(m.includes('payment_router_not_deployed'))return'Router de pagamento ainda não implantado nesta rede.';if(m.includes('wallet_missing'))return'Crie ou restaure sua wallet primeiro.';if(m.includes('insufficient_token_balance'))return'Saldo do token insuficiente.';if(m.includes('insufficient_funds'))return'Saldo insuficiente para o valor e gas.';if(m.includes('rpc_unavailable'))return'A rede selecionada está indisponível no momento.';if(m.includes('approval_failed'))return'A aprovação do token não foi confirmada.';return m.slice(0,180)}
