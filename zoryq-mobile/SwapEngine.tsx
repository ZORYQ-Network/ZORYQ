...
type BackendHealth={ok:boolean;providerPreference?:string;feeBps?:number;treasury?:string|null;treasuryConfigured?:boolean;zeroXApiConfigured?:boolean;kyberPublicConfigured?:boolean};
...
export default function SwapEngine(){
 const swapNetworks=useMemo(()=>BUILTIN_NETWORKS.filter(n=>n.mainnet&&n.supports0x),[]);
 const [networkId,setNetworkId]=useState('ethereum');
 const network=useMemo(()=>swapNetworks.find(n=>n.id===networkId)||swapNetworks[0],[swapNetworks,networkId]);
 ...
 async function bootstrap(){
  try{
   ...
   const saved=swapNetworks.find(n=>n.id===savedNetwork);if(saved)setNetworkId(saved.id);
 ...
 async function requestQuote():Promise<QuoteState>{
  ...
  const sellBase=parseUnits(amount,sell.decimals).toString();
  ...
  const out:QuoteState={envelope:j,sell,buy,sellAmount:amount,sellBase,buyDisplay:formatUnits(buyAmount,buy.decimals),feeDisplay:formatUnits(String(fee.amount),sell.decimals),feeSymbol:sell.symbol,createdAt:Date.now()};
 ...
 return <SafeAreaView style={s.root}><ScrollView contentContainerStyle={s.scroll}>
  ...
  <View style={s.amountBox}><Text style={s.amountLabel}>Quantidade</Text><Text style={s.amountValue} onPress={()=>{Alert.prompt?.('Quantidade','Digite o valor')}}>{amount||'Use os botões abaixo'}</Text><View style={s.wrap}>{['0.001','0.01','0.1','1','10','100'].map(v=><Pressable key={v} style={s.quick} onPress={()=>{setAmount(v);setQuote(null)}}><Text style={s.quickText}>{v}</Text></Pressable>)}</View></View>
...
const s=StyleSheet.create({root:{flex:1,backgroundColor:'#050609'},scroll:{padding:18,paddingBottom:42,gap:13},title:{color:'#fff',fontSize:26,fontWeight:'900'},sub:{color:'#9ba4b8',lineHeight:20},section:{color:'#fff',fontWeight:'900',fontSize:16,marginTop:4},card:{padding:15,borderRadius:16,borderWidth:1,borderColor:'#283044',backgroundColor:'#0d1119',gap:8},label:{color:'#b6bed0',fontWeight:'800'},mono:{color:'#d8d1ff',fontFamily:'monospace',fontSize:12},ok:{color:'#69e6a7',fontWeight:'800'},warning:{color:'#ffc76b',fontWeight:'800'},wrap:{flexDirection:'row',flexWrap:'wrap',gap:8},chip:{paddingHorizontal:13,paddingVertical:9,borderRadius:999,borderWidth:1,borderColor:'#343d55',backgroundColor:'#10141e'},chipOn:{backgroundColor:'#251c4d',borderColor:'#8268ff'},chipText:{color:'#fff',fontWeight:'800'},amountBox:{padding:14,borderRadius:15,backgroundColor:'#0a0d14',borderWidth:1,borderColor:'#252c3c',gap:9},amountLabel:{color:'#8d96aa',fontSize:12},amountValue:{color:'#fff',fontSize:28,fontWeight:'900'},quick:{paddingHorizontal:11,paddingVertical:7,borderRadius:10,backgroundColor:'#171c29'},quickText:{color:'#cdd3e0',fontWeight:'700'},primary:{padding:15,borderRadius:14,backgroundColor:'#704cff',alignItems:'center'},execute:{padding:15,borderRadius:14,backgroundColor:'#12a56f',alignItems:'center'},disabled:{opacity:.45},primaryText:{color:'#fff',fontWeight:'900'},quote:{padding:16,borderRadius:18,borderWidth:1,borderColor:'#436b5d',backgroundColor:'#0b1714',gap:8},quoteTitle:{color:'#7ff0bb',fontSize:18,fontWeight:'900'},row:{color:'#d6dbe6'},fee:{color:'#8ff2c3',fontWeight:'900'},note:{color:'#9099ac',fontSize:12,lineHeight:18},message:{color:'#b7c1d6',lineHeight:19},smallButton:{alignSelf:'flex-start',paddingHorizontal:10,paddingVertical:7,borderRadius:9,backgroundColor:'#171c29'},smallButtonText:{color:'#cfd6e5',fontWeight:'800',fontSize:12}});
