...
export class HeartbeatEngine {
  constructor({secret,ttlMs=DEFAULT_TTL_MS,now=()=>Date.now(),maxClockSkewMs=120_000}={}){
    if(!secret||String(secret).length<32)throw new Error('HeartbeatEngine requires a server secret');
    this.secret=String(secret);this.ttlMs=ttlMs;this.now=now;this.maxClockSkewMs=maxClockSkewMs;this.consumed=new Set();
  }
  ...
  verify({challenge,nodeId,publicKeyBase64,signatureBase64,blockSeen=-1,clientTime}){
    ...
    if(this.consumed.has(p.nonce))throw new Error('Heartbeat replay rejected');
    ...
    this.consumed.add(p.nonce);
    return Object.freeze({accepted:true,nodeId,blockSeen,protocolVersion:MOBILE_NODE_PROTOCOL_VERSION,verifiedAt:new Date(this.now()).toISOString(),xpAwarded:0});
  }
}
