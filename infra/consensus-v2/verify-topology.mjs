...
if (participantCount < 3) fail(`expected at least 3 Reth execution participants, got ${participantCount}`);
if (lighthouseCount < 3) fail(`expected at least 3 Lighthouse consensus participants, got ${lighthouseCount}`);
if (validatorCounts.length < 3 || validatorCounts.some(v => v < 1)) fail('each participant must own at least one validator');
...
if (!process.exitCode) {
  console.log(JSON.stringify({
    ok: true,
    policy: 'zoryq-consensus-v2-topology-v1',
