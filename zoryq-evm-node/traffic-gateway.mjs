...
const EDGE_HEAP_MB = Math.max(64, Number(process.env.ZORYQ_EDGE_HEAP_MB || 96));
...
const MAX_RATE_KEYS = Math.max(1000, Number(process.env.ZORYQ_MAX_RATE_KEYS || 20_000));
...
const edge = spawn(process.execPath, [`--max-old-space-size=${EDGE_HEAP_MB}`, 'edge-gateway.mjs'], { stdio: 'inherit', env: { ...process.env, PORT: String(EDGE_PORT) } });
...
function pruneWindowMap(map, cutoff) { for (const [key,value] of map) if (!value || Number(value.started||0) < cutoff) map.delete(key); }
function windowAllow(map, key, windowMs, limit) {
  const now = Date.now();
  const current = map.get(key);
  if (!current || now - current.started >= windowMs) {
    if (!current && map.size >= MAX_RATE_KEYS) {
      pruneWindowMap(map, now - windowMs);
      if (map.size >= MAX_RATE_KEYS) return false;
    }
...
setInterval(()=>{const now=Date.now();pruneWindowMap(rpcMinute,now-60_000);pruneWindowMap(faucetHour,now-3_600_000);},300_000).unref?.();
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`ZORYQ traffic gateway listening on :${PORT}; protected edge :${EDGE_PORT}; edgeHeapMb=${EDGE_HEAP_MB}`);
  console.log('ZORYQ public protection enabled', { MAX_BODY_BYTES, MAX_RPC_BATCH, GLOBAL_RPC_CONCURRENCY, PER_IP_RPC_CONCURRENCY, RPC_PER_MINUTE, FAUCET_PER_HOUR_IP, FAUCET_CONCURRENCY, FAUCET_SUCCESS_PER_HOUR, UPSTREAM_TIMEOUT_MS, MAX_RATE_KEYS, persistence:'reth-native-db' });
});
