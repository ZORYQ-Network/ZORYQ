...
import { nodeEnvWithHeap, startMemoryTelemetry } from './runtime-memory.mjs';
...
startMemoryTelemetry('product-gateway');

function child(name,command,args,env={},heapMb=128){
  const childEnv=nodeEnvWithHeap({...process.env,...env},heapMb,name);
  const p=spawn(command,args,{stdio:'inherit',env:childEnv});
