...
const PROCESS_MEMORY_STATUS=process.env.ZORYQ_PROCESS_MEMORY_STATUS||'/data/zoryq-memory-processes.json';
...
let exiting=false;
let chain=null,social=null,watchdog=null;
function shutdown(code=0){if(exiting)return;exiting=true;try{chain?.kill('SIGTERM')}catch{}try{social?.kill('SIGTERM')}catch{}try{watchdog?.kill('SIGTERM')}catch{}setTimeout(()=>process.exit(code),250).unref()}
...
chain=child('chain',process.execPath,['--max-old-space-size=192','traffic-gateway.mjs'],{PORT:String(CHAIN_PORT),ZORYQ_RETH_CHAIN_SPEC:EFFECTIVE_GENESIS,ZORYQ_ALLOW_RETH_GENESIS_RESET:allowMigratedLegacy,NODE_OPTIONS:'--max-old-space-size=128'});
social=child('social',process.execPath,['--max-old-space-size=96','social-service.mjs'],{PORT:String(SOCIAL_PORT),ZORYQ_CHAIN_BASE:`http://127.0.0.1:${CHAIN_PORT}`,NODE_OPTIONS:'--max-old-space-size=96'});
watchdog=child('memory-watchdog',process.execPath,['--max-old-space-size=32','memory-watchdog.mjs'],{ZORYQ_PROCESS_MEMORY_STATUS:PROCESS_MEMORY_STATUS,NODE_OPTIONS:'--max-old-space-size=32'});
