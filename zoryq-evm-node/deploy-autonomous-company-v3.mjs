...
const ARTIFACT = process.env.ZORYQ_AUTONOMOUS_COMPANY_V3_ARTIFACT || '/app/protocol-out/ZoryqAutonomousCompanyV3.sol/ZoryqAutonomousCompanyV3.json';
const STATE_FILE = process.env.ZORYQ_AUTONOMOUS_COMPANY_V3_STATE || '/data/zoryq-autonomous-company-v3.json';
const NAME = 'ZORYQ AI Commerce Studio';
const OBJECTIVE = 'Criar e vender produtos digitais com planejamento de um AI CEO e limites economicos on-chain';
...
if (!fs.existsSync(ARTIFACT)) throw new Error('autonomous company v3 artifact missing');

const phrase = fs.readFileSync(MNEMONIC_FILE, 'utf8').trim();
const operator = walletAt(phrase, 21, provider);
const artifact = readJson(ARTIFACT);
const abi = artifact?.abi;
let bytecode = artifact?.bytecode?.object || '';
if (!abi || !bytecode) throw new Error('invalid autonomous company v3 artifact');
if (!bytecode.startsWith('0x')) bytecode = `0x${bytecode}`;

let state = readJson(STATE_FILE) || { schema: 3, chainId: CHAIN_ID, network: 'ZORYQ EVM Testnet' };
let contract;
if (state.schema === 3 && state.contractAddress) {
...
writeJson(STATE_FILE, state);
console.log('[zoryq-company-v3] '+JSON.stringify(state));
