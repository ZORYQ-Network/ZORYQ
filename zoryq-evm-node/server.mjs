...
const MNEMONIC=process.env.ZORYQ_TESTNET_MNEMONIC;
...
if(!MNEMONIC){console.error('ZORYQ_TESTNET_MNEMONIC is required. Refusing to start with a public/default seed.');process.exit(1)}
fs.mkdirSync('/data',{recursive:true});
const anvil=spawn('anvil',['--host','127.0.0.1','--port',String(RPC_PORT),'--chain-id',String(CHAIN_ID),'--block-time','2','--accounts','1','--balance','1000000000','--mnemonic',MNEMONIC,'--state',STATE,'--state-interval','5','--preserve-historical-states'],{stdio:['ignore','pipe','pipe']});
...
async function ready(){for(let i=0;i<60;i++){try{const r=await fetch(localRpc,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',id:1,method:'eth_chainId',params:[]})});const j=await r.json();if(j.result===CHAIN_HEX){provider=new JsonRpcProvider(localRpc,CHAIN_ID,{staticNetwork:true});faucetWallet=Wallet.fromPhrase(MNEMONIC).connect(provider);return}}catch{}await sleep(500)}throw Error('EVM node did not become ready')}
