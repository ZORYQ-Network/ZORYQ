...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(req.method==='POST'&&url.pathname==='/faucet'){
   ...
   const f=loadFaucet(),k=address.toLowerCase(),last=Number(f[k]||0),now=Date.now();
   if(now-last<86400000)return send(res,429,{ok:false,error:'cooldown',nextClaim:new Date(last+86400000).toISOString()});
   const current=BigInt(await rpcLocal('eth_getBalance',[address,'latest']));
   const next=current+parseEther(FAUCET_AMOUNT);
   await rpcLocal('anvil_setBalance',[address,toBeHex(next)]);
   f[k]=now;saveFaucet(f);
 ...
 if(req.method==='POST'&&(url.pathname==='/'||url.pathname==='/rpc')){
   ...
   if(Array.isArray(raw)){
     if(raw.length===0)return send(res,200,{jsonrpc:'2.0',id:null,error:{code:-32600,message:'Invalid Request'}});
     const results=await Promise.all(raw.map(async q=>{
       if(blocked(q))return blockedReply(q);
       const upstream=await rpcProxy(q);
       if(!upstream.text)return null;
       try{return JSON.parse(upstream.text)}catch{return {jsonrpc:'2.0',id:q?.id??null,error:{code:-32603,message:'Invalid upstream response'}}}
     }));
     const replies=results.filter(Boolean);
     cors(res);
     if(replies.length===0){res.writeHead(204);return res.end()}
     res.writeHead(200,{'content-type':'application/json; charset=utf-8'});
     return res.end(JSON.stringify(replies));
   }
   if(blocked(raw)){
     const reply=blockedReply(raw);
     if(!reply){cors(res);res.writeHead(204);return res.end()}
     return send(res,200,reply)
   }
   const upstream=await rpcProxy(raw);
   cors(res);res.writeHead(upstream.status,{'content-type':'application/json; charset=utf-8'});return res.end(upstream.text)
 }
