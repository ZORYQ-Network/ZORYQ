...
function record(address,action,{proofRef='',txHash='',campaignId='genesis-v1',verification='server'}={}){const state=loadScore();state.wallets ||= {};const w=state.wallets[address] ||= {actions:{}};w.actions ||= {};let key;
  if(action==='x_share'){if(!campaignId)return {error:'campaign_required'};key=`x_share:${campaignId}`}
  else key=action;
  if(w.actions[key])return {duplicate:true,status:walletStatus(address)};
  const points=ACTION_POINTS[action];if(!points)return {error:'unsupported_action'};
  const entry={action,points,verification,campaignId,proofRef:proofRef?digest(proofRef):null,txHash:txHash||null,createdAt:Date.now()};w.actions[key]=entry;saveScore(state);return {ok:true,entry,status:walletStatus(address)}
}
...
const server=http.createServer(async(req,res)=>{try{const url=new URL(req.url||'/','http://localhost');if(req.method==='OPTIONS')return sendJson(res,204,{});
  if((req.method==='GET'||req.method==='HEAD')&&STATIC_ROUTES.has(url.pathname)){if(req.method==='HEAD'){res.writeHead(200,{'content-type':mime[path.extname(STATIC_ROUTES.get(url.pathname))]||'text/html; charset=utf-8','x-zoryq-surface':'railway-web-fallback'});return res.end()}if(serveStatic(res,STATIC_ROUTES.get(url.pathname)))return}
  ...
  if(req.method==='POST'&&url.pathname==='/genesis/action'){const raw=await readReq(req),b=raw?JSON.parse(raw):{},address=normAddress(b.address),action=String(b.action||'');if(!address)return sendJson(res,400,{ok:false,error:'invalid_address'});if(!ACTION_POINTS[action])return sendJson(res,400,{ok:false,error:'unsupported_action'});
    ...
    return sendJson(res,400,{ok:false,error:'action_not_directly_recordable'})}
  if(req.method==='POST'&&url.pathname==='/faucet')return proxyFaucet(req,res);
  return proxy(req,res)
}catch(e){console.error('public gateway error',e);return sendJson(res,500,{ok:false,error:e?.message||'internal_error'})}});
