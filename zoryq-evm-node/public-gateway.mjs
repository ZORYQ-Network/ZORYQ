...
const ACTION_POINTS=Object.freeze({x_follow:50,faucet:100,swap:250,stake:300,x_share:200});
...
const STATIC_ROUTES=new Map([
  ...
  ['/config.js', 'config.js'],['/app.js', 'app.js'],['/styles.css', 'styles.css'],['/x-follow-gate.js','x-follow-gate.js']
]);
...
function serveStatic(res,file){const full=path.join(WEB_ROOT,file);if(!full.startsWith(WEB_ROOT)||!fs.existsSync(full)||!fs.statSync(full).isFile())return false;res.writeHead(200,{'content-type':mime[path.extname(full)]||'application/octet-stream','cache-control':path.extname(full)==='.html'?'no-cache':'public, max-age=300','x-zoryq-surface':'railway-web-fallback'});fs.createReadStream(full).pipe(res);return true}
function loadScore(){try{return JSON.parse(fs.readFileSync(SCORE_FILE,'utf8'))}catch{return {version:1,wallets:{}}}}
...
function walletStatus(address){const state=loadScore(),w=state.wallets[address]||{actions:{}};const actions=Object.values(w.actions||{}).sort((a,b)=>a.createdAt-b.createdAt);const pendingTotal=actions.reduce((n,a)=>n+Number(a.points||0),0);const verifiedOnchain=actions.filter(a=>a.verification==='onchain').reduce((n,a)=>n+Number(a.points||0),0);const socialPending=actions.filter(a=>a.verification==='self-attested').reduce((n,a)=>n+Number(a.points||0),0);const genesisRequired=['x_follow','faucet','swap','stake'];const completed=genesisRequired.filter(k=>actions.some(a=>a.action===k)).length;return {ok:true,address,pendingScore:pendingTotal,verifiedOnchainScore:verifiedOnchain,socialPendingScore:socialPending,finalizedOnchainScore:0,genesis:{completed,total:genesisRequired.length,eligible:completed===genesisRequired.length,status:completed===genesisRequired.length?'pending-finalization':'in-progress'},actions}}
function record(address,action,{proofRef='',txHash='',campaignId='genesis-v1',verification='server'}={}){const state=loadScore();state.wallets ||= {};const w=state.wallets[address] ||= {actions:{}};w.actions ||= {};let key;if(action==='x_share'){if(!campaignId)return {error:'campaign_required'};key=`x_share:${campaignId}`}else key=action;if(w.actions[key])return {duplicate:true,status:walletStatus(address)};const points=ACTION_POINTS[action];if(!points)return {error:'unsupported_action'};const entry={action,points,verification,campaignId,proofRef:proofRef?digest(proofRef):null,txHash:txHash||null,createdAt:Date.now()};w.actions[key]=entry;saveScore(state);return {ok:true,entry,status:walletStatus(address)}}
...
const server=http.createServer(async(req,res)=>{try{const url=new URL(req.url||'/','http://localhost');if(req.method==='OPTIONS')return sendJson(res,204,{});if((req.method==='GET'||req.method==='HEAD')&&STATIC_ROUTES.has(url.pathname)){if(req.method==='HEAD'){res.writeHead(200,{'content-type':mime[path.extname(STATIC_ROUTES.get(url.pathname))]||'text/html; charset=utf-8','x-zoryq-surface':'railway-web-fallback'});return res.end()}if(serveStatic(res,STATIC_ROUTES.get(url.pathname)))return}if(req.method==='GET'&&url.pathname==='/genesis/status'){const address=normAddress(url.searchParams.get('address'));if(!address)return sendJson(res,400,{ok:false,error:'invalid_address'});return sendJson(res,200,walletStatus(address))}if(req.method==='POST'&&url.pathname==='/genesis/action'){const raw=await readReq(req),b=raw?JSON.parse(raw):{},address=normAddress(b.address),action=String(b.action||'');if(!address)return sendJson(res,400,{ok:false,error:'invalid_address'});if(!ACTION_POINTS[action])return sendJson(res,400,{ok:false,error:'unsupported_action'});if(SOCIAL_ACTIONS.has(action)){if(action==='x_follow'&&String(b.xHandle||'').replace(/^@/,'').toLowerCase()!=='zoriqnetwork')return sendJson(res,400,{ok:false,error:'official_x_handle_required'});if(action==='x_share'&&!String(b.proofRef||'').trim())return sendJson(res,400,{ok:false,error:'share_proof_required'});const r=record(address,action,{proofRef:b.proofRef||`${action}:${address}`,campaignId:String(b.campaignId||'genesis-v1'),verification:'self-attested'});return sendJson(res,200,{...r,notice:'Social points are pending until X OAuth/API verification is enabled.'})}if(ONCHAIN_ACTIONS.has(action)){const hash=txHash(b.txHash);if(!hash)return sendJson(res,400,{ok:false,error:'tx_hash_required'});const v=await verifyOnchain(address,action,hash);if(!v.ok)return sendJson(res,400,v);const r=record(address,action,{proofRef:hash,txHash:hash,verification:'onchain'});return sendJson(res,200,r)}return sendJson(res,400,{ok:false,error:'action_not_directly_recordable'})}if(req.method==='POST'&&url.pathname==='/faucet')return proxyFaucet(req,res);return proxy(req,res)}catch(e){console.error('public gateway error',e);return sendJson(res,500,{ok:false,error:e?.message||'internal_error'})}});
