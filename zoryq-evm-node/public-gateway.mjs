...
const STATIC_ROUTES=new Map([
  ...
  ['/project-launch', 'project-launch.html'],['/project-launch.html', 'project-launch.html'],
...
function sendJson(res,status,obj){res.writeHead(status,{'content-type':'application/json; charset=utf-8','access-control-allow-origin':'*','access-control-allow-headers':'content-type, authorization','access-control-allow-methods':'GET,POST,PATCH,DELETE,OPTIONS','cache-control':'no-store'});res.end(JSON.stringify(obj))}
...
async function readJsonReq(req,max=2_000_000){const raw=await readReq(req,max);return raw?JSON.parse(raw):{}}
...
if(url.pathname.startsWith('/factory/cloud/')){const handled=await handleFactoryCloud(req,res,{body:readJsonReq,send:sendJson});if(handled)return}
