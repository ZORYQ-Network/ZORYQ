...
import { startMemoryTelemetry } from './runtime-memory.mjs';

const PORT=Number(process.env.PORT||8085);
const RPC_URL=process.env.ZORYQ_EXPLORER_RPC_URL||'http://127.0.0.1:8082/rpc';
const MAX_TRANSACTIONS=Math.max(1000,Math.min(50000,Number(process.env.ZORYQ_EXPLORER_MAX_TRANSACTIONS||10000)));
const MAX_BLOCKS_PER_PASS=Math.max(1,Math.min(100,Number(process.env.ZORYQ_EXPLORER_MAX_BLOCKS_PER_PASS||25)));
const POLL_MS=Math.max(2000,Number(process.env.ZORYQ_EXPLORER_POLL_MS||4000));

startMemoryTelemetry('explorer-service');
...
server.listen(PORT,'127.0.0.1',()=>console.log(`ZORYQ explorer index service listening on 127.0.0.1:${PORT}`,{MAX_TRANSACTIONS,MAX_BLOCKS_PER_PASS,POLL_MS}));
