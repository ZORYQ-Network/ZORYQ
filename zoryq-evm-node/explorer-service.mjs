...
const MAX_BLOCKS_PER_PASS=Math.max(5,Number(process.env.ZORYQ_EXPLORER_MAX_BLOCKS_PER_PASS||25));
const POLL_MS=Math.max(2000,Number(process.env.ZORYQ_EXPLORER_POLL_MS||4000));
const logBand=createBandLogger('zoryq-explorer');
setInterval(logBand,30000).unref?.();
logBand();
...
const indexer=createExplorerIndexer({
  ...
  pollMs:POLL_MS,
  shouldPause:()=>governorPolicy().pauseNonEssential
});
indexer.start();
...
const server=http.createServer((req,res)=>{
  try{
    ...
    if(url.pathname==='/explorer/stats')return send(res,200,{...indexer.stats(),memory:memorySnapshot()});
    return send(res,404,{ok:false,error:'not_found'});
  }catch(error){return send(res,500,{ok:false,error:error?.message||'internal_error'})}
});

server.listen(PORT,'127.0.0.1',()=>console.log(`ZORYQ explorer index service listening on 127.0.0.1:${PORT}; memory-aware indexing enabled`));
