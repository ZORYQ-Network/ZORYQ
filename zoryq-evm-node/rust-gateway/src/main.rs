...
async fn health(state:&State)->Response<Body>{
    let chain_url=format!("http://127.0.0.1:{CHAIN_PORT}/health");
    let social_url=format!("http://127.0.0.1:{SOCIAL_PORT}/status");
    let (c,s)=tokio::join!(fetch_json(&state.client,&chain_url),fetch_json(&state.client,&social_url));
...
async fn proxy(state:&State, req:Request, port:u16, path:String)->Response<Body>{
    let (parts,body)=req.into_parts();
    let bytes=match axum::body::to_bytes(body,2*1024*1024).await{
        Ok(b)=>b,
        Err(_)=>return json_response(StatusCode::PAYLOAD_TOO_LARGE,json!({"ok":false,"error":"request_too_large"}))
    };
    let url=format!("http://127.0.0.1:{port}{path}");
    let mut rb=state.client.request(parts.method,url);
    for (k,v) in parts.headers.iter(){if k.as_str().eq_ignore_ascii_case("host"){continue} rb=rb.header(k,v);}
    let upstream=match rb.body(bytes).send().await{
        Ok(r)=>r,
        Err(e)=>return json_response(StatusCode::SERVICE_UNAVAILABLE,json!({"ok":false,"error":"upstream_unavailable","detail":e.to_string()}))
    };
...
async fn handler(axum::extract::State(state):axum::extract::State<State>,req:Request)->Response<Body>{
    ...
    if method==Method::GET && p=="/ops/process-memory" {
        return match fs::read_to_string(MEMORY_STATUS).ok().and_then(|s|serde_json::from_str::<Value>(&s).ok()){
            ...
            None=>json_response(StatusCode::SERVICE_UNAVAILABLE,json!({"ok":false,"error":"memory_watchdog_initializing"}))
        };
    }
    if method==Method::GET && matches!(p,"/social"|"/zoriq"|"/zoriq-social"|"/zoriq-social.html"){
        return match fs::read(SOCIAL_HTML){
            Ok(b)=>Response::builder().status(200).header("content-type","text/html; charset=utf-8").header("cache-control","no-cache").header("x-zoryq-product","social-rust").body(Body::from(b)).unwrap(),
            Err(_)=>json_response(StatusCode::NOT_FOUND,json!({"ok":false,"error":"social_surface_missing"}))
        };
    }
    let mut path=uri.path_and_query().map(|x|x.as_str()).unwrap_or("/").to_string();
    if p.starts_with("/api/social") {
        path=path.replacen("/api/social","",1);
        if path.is_empty(){path="/".into();}
        return proxy(&state,req,SOCIAL_PORT,path).await;
    }
    proxy(&state,req,CHAIN_PORT,path).await
}

fn spawn_node(name:&str,args:&[&str],envs:&[(&str,String)])->Child{
    let mut c=Command::new("/usr/local/bin/node");
    c.args(args).stdin(Stdio::null()).stdout(Stdio::inherit()).stderr(Stdio::inherit());
    for (k,v) in envs{c.env(k,v);}
    let child=c.spawn().unwrap_or_else(|e|panic!("failed to spawn {name}: {e}"));
    println!("[zoryq-rust] spawned {name}");
    child
}
...
async fn main(){
    ...
    let monitor=children.clone();
    tokio::spawn(async move{
        loop{
            tokio::time::sleep(std::time::Duration::from_secs(2)).await;
            let mut guard=monitor.lock().await;
            for child in guard.iter_mut(){
                if let Ok(Some(status))=child.try_wait(){eprintln!("[zoryq-rust] child exited: {status}");std::process::exit(1);}
            }
        }
    });
    ...
    println!("[zoryq-rust] native gateway listening on :{port}; release=infra-v5-rust-edge");
    let listener=tokio::net::TcpListener::bind(addr).await.expect("bind");
    let shutdown_children=children.clone();
    axum::serve(listener,app).with_graceful_shutdown(async move{
        let _=signal::ctrl_c().await;
        let mut g=shutdown_children.lock().await;
        for c in g.iter_mut(){let _=c.start_kill();}
    }).await.expect("serve");
}
