...
function checkpointCandidates(){return [
  path.join(CHECKPOINT_ROOT,'current','state.hex.gz'),
  LEGACY_CURRENT,
  path.join(CHECKPOINT_ROOT,'previous','state.hex.gz'),
  LEGACY_PREVIOUS,
  LEGACY_STATE
...
function ensureRollbackCheckpoint(source,state){
  ...
  const check=JSON.parse(gunzipSync(fs.readFileSync(TMP_ROLLBACK)).toString('utf8'));anvilAlloc(check);
  return {path:TMP_ROLLBACK,created:true};
}
function cleanupLegacyAfterStaging(rollback){
  // Preserve one verified rollback image while removing duplicate state copies that
  // caused the 500 MB volume to repeatedly approach exhaustion.
  safeRm(LEGACY_STATE);
  safeRm(LEGACY_PREVIOUS);
  safeRm(path.join(CHECKPOINT_ROOT,'previous'));
  if(rollback.created){
    safeRm(LEGACY_CURRENT);
    fs.renameSync(rollback.path,LEGACY_CURRENT);
    try{fs.chmodSync(LEGACY_CURRENT,0o600)}catch{}
  }
  // A native-RPC checkpoint is no longer used after the Reth cutover. If the
  // rollback source is the legacy current snapshot, the experimental checkpoint
  // tree can be removed to reclaim space.
...
let migration=null,rollback=null;
if(legacy.length){
  ...
  const before=volumeStats();
  console.log(`[zoryq-reth] migration preflight source=${source} bytes=${size(source)} volumeUsed=${before.percent}% freeBytes=${before.free}`);
  const state=decodeAnvilState(source),alloc=anvilAlloc(state);
  if(Object.keys(alloc).length===0)throw Error('migration produced zero accounts; refusing cutover');
  Object.assign(genesis.alloc,alloc);
  rollback=ensureRollbackCheckpoint(source,state);
  const meta=checkpointMeta(source);
...
// Stage the large effective genesis on the container filesystem first, validate it,
// and only then reclaim duplicate legacy files on /data. This avoids needing a
// second full state copy on the already-small persistent volume.
...
fs.renameSync(TMP_GENESIS,OUT);try{fs.chmodSync(OUT,0o600)}catch{}
