...
const policy = { threshold: 3, requiredRoles: roles, signers };

for (const [name, value] of Object.entries({ manifest, policy, approvals })) {
...
fs.writeFileSync(path.join(dir, 'manifest.json'), JSON.stringify(tampered, null, 2));
...
const twoApprovals = { approvals: approvals.approvals.slice(0, 2) };
fs.writeFileSync(path.join(dir, 'approvals.json'), JSON.stringify(twoApprovals, null, 2));
