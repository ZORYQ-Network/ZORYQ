...
for(const marker of ['SOCIAL SUITE','Descobrir','Mensagens','Alertas','Privacidade','Conta privada','Conquistas','Entrar com wallet','Sincronizado com Supabase + RLS','Web3/SIWE']){if(!mobileSuite.includes(marker))fail(`mobile suite marker missing: ${marker}`);else pass(`mobile suite marker: ${marker}`)}
...
for(const [name,source] of [['mobile backend',backend],['web backend',webBackend],['web sync',webSync]])for(const forbidden of ['service_role','SUPABASE_SERVICE_ROLE_KEY','sb_secret_']){if(source.includes(forbidden))fail(`${name}: secret-capability marker found: ${forbidden}`);else pass(`${name}: no client secret marker ${forbidden}`)}
