...
const files={
 ...
 migrationPrivacy:'database/migrations/20260911_zoriq_social_suite_security_and_privacy.sql',migrationRpc:'database/migrations/20260911_zoriq_social_rpc_hardening.sql',migrationProfile:'database/migrations/20260911_zoriq_social_profile_preferences_constraints.sql',migrationReports:'database/migrations/20260911_zoriq_social_reporting_and_hides.sql',migrationUpsert:'database/migrations/20260911_zoriq_social_upsert_update_policies.sql',migrationCards:'database/migrations/20260911_zoriq_social_public_profile_cards_and_interaction_visibility.sql',migrationProfileReads:'database/migrations/20260911_zoriq_social_restrict_profile_direct_reads.sql'
};
...
const migrationPrivacy=read('migrationPrivacy'),migrationRpc=read('migrationRpc'),migrationProfile=read('migrationProfile'),migrationReports=read('migrationReports'),migrationUpsert=read('migrationUpsert'),migrationCards=read('migrationCards'),migrationProfileReads=read('migrationProfileReads');
...
for(const marker of ['ZORYQ_SOCIAL_BACKEND','grant_type=web3','PUBLISHABLE_KEY','signInWithWallet','refresh_token','listProfiles','listProfileCards','social_public_profiles','createPost','conversations','notifications','startPolling']){if(!webBackend.includes(marker))fail(`web backend marker missing: ${marker}`);else pass(`web backend marker: ${marker}`)}
for(const marker of ['SUPABASE + RLS','Web ↔ APK','Entrar com wallet','toggleFollow','createPost','sendMessage','markAllNotifications','listProfileCards']){if(!webSync.includes(marker))fail(`web sync marker missing: ${marker}`);else pass(`web sync marker: ${marker}`)}
...
for(const marker of ['SOCIAL SUITE','Rede','Pessoas','Mensagens','Alertas','Privacidade','Conta privada','Conquistas','Entrar com wallet','Sincronizado com Supabase + RLS','Web3/SIWE','SyncedFeed']){if(!mobileSuite.includes(marker))fail(`mobile suite marker missing: ${marker}`);else pass(`mobile suite marker: ${marker}`)}
...
for(const marker of ['Wallet / Recovery','ZORIQ SECURITY','Confirmar transações','BIOMETRIC_STRONG','SocialSuite','getBackendState','listVisibleFeed','Social backend','Notificações sociais','Ativar com esta wallet','listNotifications','markAllNotificationsRead','subscribeSocial']){if(!mobileIndex.includes(marker))fail(`mobile shell/security/notification marker missing: ${marker}`);else pass(`mobile shell/security/notification marker: ${marker}`)}
for(const marker of ['createClient','EXPO_PUBLIC_SUPABASE_URL','EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY','persistSession:true','signInWithLocalWallet','signInWithWeb3','getMyProfile','listProfileCards','social_public_profiles','listDiscoverableProfiles','listConversations','listMessages','markAllNotificationsRead','social_toggle_follow','social_add_comment','start_direct_conversation','subscribeSocial','reportContent','hidePost']){if(!backend.includes(marker))fail(`mobile backend marker missing: ${marker}`);else pass(`mobile backend marker: ${marker}`)}

for(const [name,source] of [['mobile backend',backend],['web backend',webBackend],['web sync',webSync],['mobile synced feed',mobileFeed],['mobile shell',mobileIndex]])for(const forbidden of ['service_role','SUPABASE_SERVICE_ROLE_KEY','sb_secret_']){if(source.includes(forbidden))fail(`${name}: secret-capability marker found: ${forbidden}`);else pass(`${name}: no client secret marker ${forbidden}`)}

for(const marker of ['follow_requests','user_mutes','posts_visible_read','supabase_realtime']){if(!migrationPrivacy.includes(marker))fail(`privacy migration marker missing: ${marker}`);else pass(`privacy migration: ${marker}`)}
for(const marker of ['can_view_social_post','social_toggle_like','social_toggle_follow','social_accept_follow_request','start_direct_conversation','revoke all']){if(!migrationRpc.includes(marker))fail(`RPC migration marker missing: ${marker}`);else pass(`RPC migration: ${marker}`)}
for(const marker of ['show_reputation','profiles_human_score_range','profiles_dm_mode_valid','profiles_power_key_valid']){if(!migrationProfile.includes(marker))fail(`profile migration marker missing: ${marker}`);else pass(`profile migration: ${marker}`)}
for(const marker of ['content_reports','post_hides','content_reports_own_insert','post_hides_own_insert']){if(!migrationReports.includes(marker))fail(`moderation migration marker missing: ${marker}`);else pass(`moderation migration: ${marker}`)}
for(const marker of ['content_reports_own_update','user_mutes_own_update','grant update']){if(!migrationUpsert.includes(marker))fail(`upsert migration marker missing: ${marker}`);else pass(`upsert migration: ${marker}`)}
for(const marker of ['private.social_public_profiles','public.social_public_profiles','security invoker','post_reactions_visible_read','post_reposts_visible_read']){if(!migrationCards.includes(marker))fail(`public-card migration marker missing: ${marker}`);else pass(`public-card migration: ${marker}`)}
for(const marker of ['profiles_public_read','profiles_self_read','auth.uid()']){if(!migrationProfileReads.includes(marker))fail(`profile read restriction marker missing: ${marker}`);else pass(`profile read restriction: ${marker}`)}
for(const marker of ['offline-first','publishable key only','Row-Level Security','Hardened RPCs','content_reports','Sign-In With Ethereum','global notification center','social_public_profiles','Full profile rows are readable only']){if(!backendDoc.includes(marker))fail(`backend documentation marker missing: ${marker}`);else pass(`backend documentation: ${marker}`)}

const forbiddenClaims=[/production-ready social network/i,/guaranteed bot[- ]free/i,/fully decentralized social network/i,/AI[^\n.]{0,50}live globally/i,/end-to-end encrypted messages are live/i,/push notifications are live/i];
for(const pattern of forbiddenClaims){if(pattern.test(social)||pattern.test(js)||pattern.test(suiteJs)||pattern.test(webSync)||pattern.test(mobileIndex))fail(`unverified capability claim matched ${pattern}`);else pass(`no unverified claim ${pattern}`)}
