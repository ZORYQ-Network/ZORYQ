...
const files={
 social:'zoryq-web/zoriq-social-v2.html',css:'zoryq-web/zoriq-social-v2.css',js:'zoryq-web/zoriq-social-v2.js',suiteCss:'zoryq-web/zoriq-social-suite.css',suiteJs:'zoryq-web/zoriq-social-suite.js',
 mobile:'zoryq-mobile/Social.tsx',mobileSuite:'zoryq-mobile/SocialSuite.tsx',mobileBackend:'zoryq-mobile/socialBackend.ts',mobileIndex:'zoryq-mobile/index.js',mobilePackage:'zoryq-mobile/package.json',
...
const migrationPrivacy=read('migrationPrivacy'),migrationRpc=read('migrationRpc'),migrationProfile=read('migrationProfile'),migrationReports=read('migrationReports');
const vercel=JSON.parse(read('vercel'));

for(const marker of ['ZORIQ Social','YOUR SOCIAL WORLD EVOLVES WITH YOU','ZORIQ PULSE','ZORIQ WORLDS','PERFIL EVOLUTIVO','ZORIQ DNA','ZORIQ POWERS','REPUTATION GRAPH','HUMAN FIRST','ZORI · COMPANHEIRO SOCIAL','maxlength="500"','zoriq-social-suite.css','zoriq-social-suite.js']){if(!social.includes(marker))fail(`social marker missing: ${marker}`);else pass(`social marker: ${marker}`)}
...
for(const marker of ['SOCIAL SUITE','Descobrir','Mensagens','Alertas','Privacidade','Conta privada','Conquistas']){if(!mobileSuite.includes(marker))fail(`mobile suite marker missing: ${marker}`);else pass(`mobile suite marker: ${marker}`)}
...
for(const marker of ['createClient','EXPO_PUBLIC_SUPABASE_URL','EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY','persistSession:true','listDiscoverableProfiles','listVisibleFeed','social_toggle_follow','social_add_comment','start_direct_conversation','subscribeSocial','reportContent','hidePost','post_hides','content_reports']){if(!backend.includes(marker))fail(`backend adapter marker missing: ${marker}`);else pass(`backend adapter marker: ${marker}`)}
for(const forbidden of ['service_role','SUPABASE_SERVICE_ROLE_KEY','sb_secret_']){if(backend.includes(forbidden))fail(`secret-capability marker found in client backend: ${forbidden}`);else pass(`no client secret marker: ${forbidden}`)}
...
for(const pattern of forbiddenClaims){if(pattern.test(social)||pattern.test(js)||pattern.test(suiteJs))fail(`unverified capability claim matched ${pattern}`);else pass(`no unverified claim ${pattern}`)}
