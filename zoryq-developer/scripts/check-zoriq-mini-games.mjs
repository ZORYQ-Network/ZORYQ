...
const files={games:'zoryq-mobile/MiniGames.tsx',bridge:'zoryq-mobile/SocialSuite.native.js',package:'zoryq-mobile/package.json',social:'zoryq-mobile/Social.tsx',app:'zoryq-mobile/app.json',payment:'database/migrations/20260911_zoriq_social_payment_destination.sql'};
...
for(const marker of ['😂 Divertir','🎮 Games','MiniGames',"['home','⌂','Home']","['search','⌕','Buscar']","['create','＋','Criar']","['profile','◎','Perfil']",'$ Enviar cripto','social_payment_destination','Alterar foto','☀ Dia','☾ Noite','◐ Automático','expo-image-picker']){if(!bridge.includes(marker))fail(`native experience marker missing: ${marker}`);else pass(`native experience: ${marker}`)}
if(bridge.includes("['network','Rede']"))fail('obsolete Rede navigation tab is still present');else pass('Rede navigation tab removed');
