...
const socialPath='zoryq-web/zoryq-social.html';
...
const deployPath='zoryq-developer/ZORYQ_SOCIAL_DEPLOY.md';
...
for(const route of ['/zoryq','/social','/zoryq-social']){
  if(rewriteMap.get(route)!=='/zoryq-social.html') fail(`missing deploy rewrite for ${route}`); else pass(`deploy rewrite: ${route}`);
}
...
if(!social.includes('ZORYQ never asks for a seed phrase or private key')) fail('wallet safety language missing'); else pass('wallet safety language present');
...
if(process.exitCode) process.exit(process.exitCode);
console.log('ZORYQ Social deployment-readiness static checks passed.');
