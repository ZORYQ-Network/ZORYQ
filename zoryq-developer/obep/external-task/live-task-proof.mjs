...
import { Wallet, JsonRpcProvider, hexlify, parseEther, toUtf8Bytes } from 'ethers';
import { createIntentEnvelope, authorizeIntent, recordExecution, verifyOutcome, ObepLedger, exportProofPack, hashObject, verifyProofPack } from '../obep.mjs';
...
// Security boundary: live network evidence is always written to this repository-local path.
// No CLI/environment input is permitted to influence the destination.
const OUT = path.resolve(process.cwd(), 'zoryq-evidence', 'obep', 'external-task-live-proof.json');
...
async function main() {
  ...
  const claimResponse = await fetch(`${BASE}/faucet/claim`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ address: treasury.address }) });
  ...
  const intent = createIntentEnvelope({ companyId: `company:obep-useful-task:${treasury.address.toLowerCase()}`, goal: 'Compute, independently verify, and settle a deterministic useful dataset-summary task on ZORYQ Testnet', task, executor: executor.address, verifier: verifier.address, treasury: treasury.address, budget: PAYMENT_WEI, chainId: CHAIN_ID, nonce: `useful-task-${Date.now()}-${treasury.address.toLowerCase()}`, validFrom: now - 30, validUntil: now + 900, policy: { maxPayment: PAYMENT_WEI, asset: 'ZQ_TESTNET_NATIVE', capability: 'ZORYQ_OBEP_EXTERNAL_TASK_V1', revocable: true } });
  ...
  const evidence = { protocol: 'ZORYQ_OBEP_USEFUL_TASK_LIVE_PROOF', version: '0.2.0', generatedAt: new Date().toISOString(), rpc: RPC, chainId: CHAIN_ID, taskVerification: { specification: task.specification, inputDigest: taskResult.inputDigest, taskOutputHash: taskResult.outputHash, recomputationAccepted: verification.accepted }, proofPack, roleAttestations: { authorization: { address: treasury.address, message: authorizationMessage, signature: authorizationSignature }, execution: { address: executor.address, message: executionMessage, signature: executionSignature }, outcome: { address: verifier.address, message: outcomeMessage, signature: outcomeSignature } }, onchainBinding: { binding, commitment, calldata, txHash: tx.hash, blockNumber: rpcReceipt.blockNumber, blockHash: rpcReceipt.blockHash, from: rpcTx.from, to: rpcTx.to, value: rpcTx.value.toString(), status: rpcReceipt.status, faucetFundingTxHash: claim.txHash }, securityBoundary: { privateKeysPersisted: false, privateKeysEmitted: false, rolesIndependentWithinProof: true, externalHumanOrProjectIndependence: false, walletType: 'ephemeral-ci-only', network: 'public-centralized-testnet', productionClaim: false } };
...
main().catch((error) => { console.error(JSON.stringify({ ok: false, protocol: 'ZORYQ_OBEP_USEFUL_TASK_LIVE_PROOF', error: error.message })); process.exit(1); });